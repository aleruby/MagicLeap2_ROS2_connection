...
public class Eye_Nasal_Right_Definitive : MonoBehaviour
{
    [HideInInspector]
    ...
    private string sensor_Name = "Eye Nasal Right";
    ...
    void Update()
    {
        if (turn_on)
        {
            topicName = topic_base_name + "/eye_nasal_right/image_raw";

            topicNameIntrinsecs = topic_base_name + "/eye_nasal_right/camera_info";

            topicNameMetaData = topic_base_name + "/eye_nasal_right/metatdata";
            ...
            {
                msg = new StringMsg("eye_nasal_right: Magic Leap Pixel Sensor Feature is not available or not enabled.");
            ...
            msg = new StringMsg($"eye_nasal_right: Sensor creation was successful: {wasCreated}");
            ros.Publish(topic_base_name + "/log", msg);
            ...
            for (uint streamIndex = 0; streamIndex < numberOfStreams; streamIndex++)
            {
                ...
                {
                    msg = new StringMsg($"eye_nasal_right: Failed to get capabilities for stream index {streamIndex}");
                    ros.Publish(topic_base_name + "/log", msg);
                }
            }
        ...
        private IEnumerator StartSensorStream()
    {
        ...
        {
            msg = new StringMsg("eye_nasal_right: Sensor configuration successful.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        else
        {
            msg = new StringMsg("eye_nasal_right: Sensor configured successfully.");
            ros.Publish(topic_base_name + "/log", msg);
            yield break;
        }
        ...
        if (pixelSensorFeature.EnumeratePixelSensorMetaDataTypes(sensorType, (uint) stream_Index, out var metaDataTypes))
        {
            ...
            msg = new StringMsg($"eye_nasal_right: Metadata types for stream {(uint) stream_Index} retrieved successfully.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        else
        {
            msg = new StringMsg($"eye_nasal_right: Failed to retrieve metadata types for stream {(uint) stream_Index}.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        ...
        {
            msg = new StringMsg("eye_nasal_right: Stream could not be started.");
            ros.Publish(topic_base_name + "/log", msg);
            yield break;
        }

        msg = new StringMsg("eye_nasal_right: Stream started successfully.");
        ros.Publish(topic_base_name + "/log", msg);
    ...
    private void SendFrameROS(in PixelSensorFrame frame, int sec, uint nanosec)
    {
        var message = new ImageMsg
        {
            ...
            { 
                frame_id = "eye_nasal_right_optical_frame",
    ...
    private IEnumerator StopSensorCoroutine()
    {
        ...
        if (sensorStopAsyncResult.DidOperationSucceed)
        {
            ...
            if (wasDestroyed)
            {
                msg = new StringMsg("eye_nasal_right: Sensor destroyed.");
                ros.Publish(topic_base_name + "/log", msg);
            }
            msg = new StringMsg("eye_nasal_right: Sensor streaming stopped successfully.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        else
        {
            msg = new StringMsg("eye_nasal_right: Failed to stop sensor streaming.");
            ros.Publish(topic_base_name + "/log", msg);
        }
    }
}
