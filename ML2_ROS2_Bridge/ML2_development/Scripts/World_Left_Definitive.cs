...
public class World_Left_Definitive : MonoBehaviour
{
    [HideInInspector]
    ...
    private string sensor_Name = "World Left";
    ...
    void Update()
    {
        if (turn_on)
        {
            topicName = topic_base_name + "/world_left/image_raw";

            topicNameIntrinsecs = topic_base_name + "/world_left/camera_info";

            topicNameMetaData = topic_base_name + "/world_left/metadata";
            ...
            {
                msg = new StringMsg("world_left: Magic Leap Pixel Sensor Feature is not available or not enabled.");
            ...
            msg = new StringMsg($"world_left: Sensor creation was successful: {wasCreated}");
            ros.Publish(topic_base_name + "/log", msg);
            ...
            for (uint streamIndex = 0; streamIndex < numberOfStreams; streamIndex++)
            {
                ...
                {
                    msg = new StringMsg($"world_left: Failed to get capabilities for stream index {streamIndex}");
                    ros.Publish(topic_base_name + "/log", msg);
                }
            }
    ...
    private IEnumerator StartSensorStream()
    {
        ...
        {
            msg = new StringMsg("world_left: Sensor configuration successful.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        else
        {
            msg = new StringMsg("world_left: Sensor configuration failed.");
            ros.Publish(topic_base_name + "/log", msg);
            yield break;
        }
        msg = new StringMsg("world_left: Sensor configured successfully.");
        ros.Publish(topic_base_name + "/log", msg);
        ...
        if (pixelSensorFeature.EnumeratePixelSensorMetaDataTypes(sensorType, (uint) stream_Index, out var metaDataTypes))
        {
            ...
            msg = new StringMsg($"world_left: Metadata types for stream {(uint) stream_Index} retrieved successfully.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        else
        {
            msg = new StringMsg($"world_left: Failed to retrieve metadata types for stream {(uint) stream_Index}.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        ...
        {
            msg = new StringMsg("world_left: Stream could not be started.");
            ros.Publish(topic_base_name + "/log", msg);
            yield break;
        }
        msg = new StringMsg("world_left: Stream started successfully.");
        ros.Publish(topic_base_name + "/log", msg);
    ...
    private void SendFrameROS(in PixelSensorFrame frame, int sec, uint nanosec)
    {
        var message = new ImageMsg
        {
            ...
            { 
                frame_id = "world_left_optical_frame",
    ...
    private void SendPoseROS(Pose sensorPose, int sec, uint nanosec)
    {
        TransformStampedMsg transform0 = new TransformStampedMsg
        {
            ...
            child_frame_id = "world_left_frame",
        ...
        TransformStampedMsg transform1 = new TransformStampedMsg
        {
            header = new RosMessageTypes.Std.HeaderMsg 
            { 
                frame_id = "world_left_frame",
                stamp = new RosMessageTypes.BuiltinInterfaces.TimeMsg(sec, nanosec)
            },
            child_frame_id = "world_left_optical_frame",
            transform = new TransformMsg
            {
                translation = new Vector3Msg
                {
    ...
    private IEnumerator StopSensorCoroutine()
    {
        ...
        if (sensorStopAsyncResult.DidOperationSucceed)
        {
            ...
            if (wasDestroyed)
            {
                msg = new StringMsg("world_left: Sensor destroyed.");
                ros.Publish(topic_base_name + "/log", msg);
            }
            msg = new StringMsg("world_left: Sensor streaming stopped successfully.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        else
        {
            msg = new StringMsg("world_left: Failed to stop sensor streaming.");
            ros.Publish(topic_base_name + "/log", msg);
        }
    }
}
