...
public class World_Center_Definitive : MonoBehaviour
{
    [HideInInspector]
    ...
    private string sensor_Name = "World Center";
    ...
    public enum stream : uint { stream0 = 0,  stream1 = 1};
    public stream stream_Index;
    public enum possible_resolution : uint { res1016x1016 = 0};
    ...
    {
        // Qui chiama il metodo sull'istanza interna "sensor"
        sensor.GetSensorPose(frame.CaptureTime);
    }*/                 
    ...
    void Update()
    {
        if (turn_on)
        {
            topicName = topic_base_name + "/world_center/image_raw";

            topicNameIntrinsecs = topic_base_name + "/world_center/camera_info";

            topicNameMetaData = topic_base_name + "/world_center/metadata";
            ...
            if (resolution == 0)
            {
                ResolutionWidth = 1016;
                ResolutionHeight = 1016;
            }
            ...
            {
                msg = new StringMsg("world_center: Magic Leap Pixel Sensor Feature is not available or not enabled.");
            ...
            msg = new StringMsg($"world_center: Sensor creation was successful: {wasCreated}");
            ros.Publish(topic_base_name + "/log", msg);
            ...
            for (uint streamIndex = 0; streamIndex < numberOfStreams; streamIndex++)
            {
                ...
                {
                    msg = new StringMsg($"world_center: Failed to get capabilities for stream index {streamIndex}");
                    ros.Publish(topic_base_name + "/log", msg);
                }
            }
    ...
    private IEnumerator StartSensorStream()
    {
        ...
        {
            msg = new StringMsg("world_center: Sensor configuration successful.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        else
        {
            msg = new StringMsg("world_center: Sensor configuration failed.");
            ros.Publish(topic_base_name + "/log", msg);
            yield break;
        }
        msg = new StringMsg("world_center: Sensor configured successfully.");
        ros.Publish(topic_base_name + "/log", msg);
        ...
        if (pixelSensorFeature.EnumeratePixelSensorMetaDataTypes(sensorType, (uint) stream_Index, out var metaDataTypes))
        {
            ...
            msg = new StringMsg($"world_center: Metadata types for stream {(uint) stream_Index} retrieved successfully.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        else
        {
            msg = new StringMsg($"world_center: Failed to retrieve metadata types for stream {(uint) stream_Index}.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        ...
        {
            msg = new StringMsg("world_center: Stream could not be started.");
            ros.Publish(topic_base_name + "/log", msg);
            yield break;
        }
        msg = new StringMsg("world_center: Stream started successfully.");
        ros.Publish(topic_base_name + "/log", msg);
    ...
    private void SendFrameROS(in PixelSensorFrame frame, int sec, uint nanosec)
    {
        var message = new ImageMsg
        {
            ...
            { 
                frame_id = "world_center_optical_frame",
    ...
    private IEnumerator StopSensorCoroutine()
    {
        ...
        if (sensorStopAsyncResult.DidOperationSucceed)
        {
            ...
            if (wasDestroyed)
            {
                msg = new StringMsg("world_center: Sensor destroyed.");
                ros.Publish(topic_base_name + "/log", msg);
            }
            msg = new StringMsg("world_center: Sensor streaming stopped successfully.");
            ros.Publish(topic_base_name + "/log", msg);
        }
        else
        {
            msg = new StringMsg("world_center: Failed to stop sensor streaming.");
            ros.Publish(topic_base_name + "/log", msg);
        }
    }
}
